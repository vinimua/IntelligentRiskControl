...
class WorkflowRepo:
    ...
    async def update_phase(self, lifecycle_run_id: str, phase: str, state_json: dict | None = None) -> None:
        ...
        if state_json is not None:
            sets.append("state_json = CAST(:state AS JSONB)")
    ...
    async def log_action(
        ...
        await self.session.execute(
            text("""
                ...
                VALUES (:rid, :node, :phase, :action, :status, :dur, CAST(:summary AS JSONB), CAST(:error AS JSONB))
            """),
            {
    ...
    async def push_outbox(
        ...
        await self.session.execute(
            text("""
                ...
                VALUES (:id, :rid, :type, CAST(:payload AS JSONB), :key)
            """),
            {
                "id": new_id,
                "rid": lifecycle_run_id,
    ...
    async def receive_inbox(
        ...
        await self.session.execute(
            text("""
                ...
                VALUES (:id, :rid, :key, :type, CAST(:payload AS JSONB), TRUE)
            """),
            {
                "id": new_id,
                "rid": lifecycle_run_id,
